...
def get_M_R(EoS_str, dm_om, M_DM):
    ...
    for ii in range(len(res)):
        res[ii][1] = con_radius*res[ii][1]
        res[ii][2] = con_radius*res[ii][2]
        res[ii][3] = con_mass*res[ii][3]
        res[ii][4] = con_mass*res[ii][4]
    return res

def get_stable(EoS_str, dm_om, M_DM):
    M_R = get_M_R(EoS_str, dm_om, M_DM)  
...
def get_stable_M_R_pure():
    ...
    con_mass = pow(Planck_Mass, 3)*pow(mass_DM*MeV_solar, -2)
    con_radius = pow(Planck_Mass, 2)*pow(mass_DM*MeV_solar, -2)*Planck_Length               #1.769 * 1e-76# *neutron_mass_fm**2
    M_R_OM = np.genfromtxt("results/full_EoS_%s_run/M_R_TIDAL_P_dm_om_%1.3e_M_%1.3e_2_fluid.out"%(EoS_str,0, 1000))
    M_R_DM = np.genfromtxt("results/full_EoS_%s_run/M_R_TIDAL_P_dm_om_%1.3e_M_%1.3e_2_fluid.out"%(EoS_str,-1, 1000))
    for ii in range(len(M_R_OM)):
        M_R_OM[ii][1] = con_radius*M_R_OM[ii][1]
        M_R_OM[ii][2] = con_radius*M_R_OM[ii][2]
        M_R_OM[ii][3] = con_mass*M_R_OM[ii][3]
        M_R_OM[ii][4] = con_mass*M_R_OM[ii][4]
        M_R_DM[ii][1] = con_radius*M_R_DM[ii][1]
        M_R_DM[ii][2] = con_radius*M_R_DM[ii][2]
        M_R_DM[ii][3] = con_mass*M_R_DM[ii][3]
        M_R_DM[ii][4] = con_mass*M_R_DM[ii][4]
    tmp_OM = np.genfromtxt("results/full_EoS_%s_run/stability_P_dm_om_%1.3e_M_%1.3e_2_fluid"%(EoS_str,0, 1000))[1:]
    tmp_DM = np.genfromtxt("results/full_EoS_%s_run/stability_P_dm_om_%1.3e_M_%1.3e_2_fluid"%(EoS_str,-1, 1000))[1:]
    stability_OM = []
    for k in range(len(tmp_OM)):
        if (tmp_OM[k][0] < EoS_max(mass_DM)):
            if (tmp_OM[k][1] <= EoS_max(mass_DM)):
                stability_OM.append(list(tmp_OM[k]))
            else:
                stability_OM.append([tmp_OM[k][0],EoS_max(mass_DM)])
    for m in range(len(stability_OM)):               # num of stable in branch
        M_R_stable_OM.append([])
        for k in range(len(M_R_OM[0])):              # num of obs  
            M_R_stable_OM[m].append([])
            for l in range(len(M_R_OM)):             # num of P0
                if M_R_OM[l][0] > stability_OM[m][0] and M_R_OM[l][0] < stability_OM[m][1]:
                    M_R_stable_OM[m][k].append(M_R_OM[l][k])
    stability_DM = []
    for k in range(len(tmp_OM)):
        if (tmp_OM[k][0] < EoS_max(mass_DM)):
            if (tmp_OM[k][1] <= EoS_max(mass_DM)):
                stability_DM.append(list(tmp_OM[k]))
            else:
                stability_DM.append([tmp_OM[k][0],EoS_max(mass_DM)])
    for m in range(len(stability_DM)):               # num of stable in branch
        M_R_stable_DM.append([])
        for k in range(len(M_R_DM[0])):              # num of obs  
            M_R_stable_DM[m].append([])
            for l in range(len(M_R_DM)):             # num of P0
                if M_R_DM[l][0] > stability_DM[m][0] and M_R_DM[l][0] < stability_DM[m][1]:
                    M_R_stable_DM[m][k].append(M_R_DM[l][k])
    return M_R_stable_OM, M_R_stable_DM
...
def get_data(M_R_stable):
    """
    Extracts data from M_R_stable of the form [obs][p0] and translates it to a dicc
    """
    res = {}
...
def plot_M_R_dm_om(data, M_ind, show=True):
...
if __name__ == "__main__":
    ...
    stable_pure_OM, stable_pure_DM = get_stable_M_R_pure()
    stable_c_pure_OM = combine_stables(stable_pure_OM)
    data_OM = get_data(stable_c_pure_OM)

    plt.plot(data_OM["R_max"], data_OM["M_sum"])
    # plot_3D(data["M_max"],"M_max")
    plot_M_R_dm_om(data, 14, show = True)
