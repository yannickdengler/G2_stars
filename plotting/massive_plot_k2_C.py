...
def get_data(file):
    C = np.genfromtxt(file, usecols = 11) # OM matter radius
    k2 = np.genfromtxt(file, usecols = 12) # OM matter radius
...
def do_four_plots(axs, EoS):
    ...
    ax1.text(xtext, ytext, r'$m_{C} = %0.1f\,\text{GeV}$'%(0.5), ha='left', va='bottom', transform=ax1.transAxes, fontsize=fontsize, bbox=props)
    ax2.text(xtext, ytext, r'$m_{C} = %i\,\text{GeV}$'%(1), ha='left', va='bottom', transform=ax2.transAxes, fontsize=fontsize, bbox=props)
    ax3.text(xtext, ytext, r'$m_{C} = %i\,\text{GeV}$'%(2), ha='left', va='bottom', transform=ax3.transAxes, fontsize=fontsize, bbox=props)
    ax4.text(xtext, ytext, r'$m_{C} = %i\,\text{GeV}$'%(4), ha='left', va='bottom', transform=ax4.transAxes, fontsize=fontsize, bbox=props)
...
def plot_M_R_massive(show = False):
    ...
    for i in range(len(EoSs)):
        ...
        axss[i,0].set_yticks(yticks, [r"$%1.2f$"%x for x in yticks])
        axss[i,0].set_ylabel(r"$k_2$", fontsize = fontsize)
        axss[i,3].text(xtext, ytext, r'$\text{%s}$'%EoS_name(EoSs[i]), ha='right', va='top', transform=axss[i,3].transAxes, fontsize=fontsize, bbox=props)

    for tmp in np.transpose(axss):
        xticks = [0.05,0.1,0.15,0.2,0.25]
        xticks_lab = [r"$0.05$",r"",r"$0.15$",r"",r"$0.25$"]
        # tmp[4].set_xticks(xticks, [r"$%1.2f$"%x for x in xticks])
        tmp[4].set_xticks(xticks, xticks_lab)
        tmp[4].set_xlabel(r"$\text{C}$", fontsize = fontsize)
    ...
    fig.savefig('plots/k2_C_massive.pdf', bbox_inches = "tight")
    fig.savefig('plots/k2_C_massive.png', bbox_inches = "tight", dpi=300)
    if show:
        plt.show()
    plt.close(fig)
