...
def plot_M_R_big(EoS, show = False):
    ...
    ax3.set_xlabel(r"$\text{C}$", fontsize = styles.fontsize)
    ax4.set_xlabel(r"$\text{C}$", fontsize = styles.fontsize)
    ax1.set_ylabel(r"$k_2$", fontsize = styles.fontsize)
    ax3.set_ylabel(r"$k_2$", fontsize = styles.fontsize)
    xticks = np.linspace(0,0.2,3)
    ax3.set_xticks(xticks, [r"$%1.1f$"%x for x in xticks])
    xticks = np.linspace(0,0.3,4)
    ax4.set_xticks(xticks, [r"$%1.1f$"%x for x in xticks])
    yticks = np.linspace(00.05,0.2,4)
    ax1.set_yticks(yticks, [r"$%1.2f$"%x for x in yticks])
    ax3.set_yticks(yticks, [r"$%1.2f$"%x for x in yticks])
    ...
    ax1.legend(loc="upper right",title = r"$\text{%s}$"%EoS_name(EoS), fontsize = styles.fontsize-2)
    fig.savefig('plots/k_2_C_big.pdf', bbox_inches = "tight")
    if show:
        plt.show()
    plt.close(fig)
